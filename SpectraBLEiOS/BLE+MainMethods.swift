...
//            }
            ...
            let data:DeviceData = DeviceData(deviceName: self.connectedDevice?.deviceName ?? "", deviceType: self.connectedDevice?.deviceType ?? "", deviceID: self.connectedDevice?.peripheralId ?? "", punchTime: formattedDate)
    ...
    public func startScan(callback: @escaping (BLEEvent, Any) -> Void) {
        ...
        scanQueue.async { [weak self] in
            ...
            self.centralManager?.scanForPeripherals(withServices: [CBUUID(string: DeviceConstant.CDeviceUDID)], options: [CBCentralManagerScanOptionAllowDuplicatesKey: true])
    ...
    public func getDeviceList(callback: @escaping (BLEEvent, Any) -> Void) {
        ...
        for device in scannedDevices {
            ...
            let data:DeviceData = DeviceData(deviceName: (device.deviceName ?? ""), deviceType: (device.deviceType ?? ""), deviceID: (device.peripheralId ?? ""), punchTime: "")
            arrDevices.append(data)
        }
